...
public class DispatchBatchRequest : DispatchBatchRequest<BatchedRoutedTypedNotice>
{
  ...
  /// </exception>
  public static new DispatchBatchRequest CreateFromRoutedNotices(
  ...
  )
  {
    ...
    Dictionary<string, BatchedRoutedTypedNotice> MakeDictionary()
    {
      try
      {
    ...
    BatchDispatchOptions? options = null)
  {
    return CreateFromRoutedNotices(
      notices.Select(notice => new BatchedRoutedTypedNotice(stream, notice)),
      batchIdProvider,
      options
    );
  }
