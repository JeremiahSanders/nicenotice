# NoticeRouter&lt;TEnterpriseEventBaseType&gt;.GetStreamId&lt;TEventType&gt; method
...
Note that [`EventStreamId`](../../Jds.NiceNotice/EventStreamId.md) is a logical token; it is not a direct reference to a physical stream. Notice dispatch I/O implementations determine how the logical stream is mapped to a physical stream (e.g., an SNS topic).
