...
public class GenericDispatcher_DirectConstruction_DispatchBatchAsync_Assertions(
  GenericDispatcher_DirectConstruction_DispatchBatchAsync_Assertions.Fixture fixture
)
  : BaseCaseAssertions<GenericDispatcher_DirectConstruction_DispatchBatchAsync_Assertions.Fixture>(fixture)
{
  [Fact]
  public void Act_ResponseIndicatesSuccess()
  {
    //   All the notices should have been returned as a success.
    CaseArrangement.request.Notices.ShouldAllBe(kvp =>
      CaseArrangement.result.Successes.Any(response => response.BatchNoticeId == kvp.Key)
    );
  }
